...
test('ordinary UI state changes never hard-reload the document', () => {
  const files = [
    ...
    'setup-selection-ui.js',
    'loadout-capabilities-ui.js',
  ];
  for (const file of files) {
    const source = read(file);
    assert.doesNotMatch(source, /(?:window\.)?location\.reload\s*\(/, `${file} still reloads the page`);
    assert.doesNotMatch(source, /window\.location\.(?:replace|assign)\s*\(/, `${file} still navigates the whole document`);
  }
});
...
test('backup restore and local reset repaint without a hard reload', () => {
  ...
  assert.doesNotMatch(ordinarySettingsWrites, /location\.(?:reload|replace|assign)\s*\(/);
  assert.match(ordinarySettingsWrites, /farming420:state-changed/);
});

test('navigation cleanup and reforge edits publish state changes instead of navigating', () => {
  const nav = read('navigation-dedupe.js');
  assert.doesNotMatch(nav, /location\.reload\s*\(/);
  assert.match(nav, /farming420:state-changed/);

  const redesign = read('skyblock-redesign.js');
  assert.match(redesign, /writeState\(state\);\s*window\.dispatchEvent\(new Event\('farming420:state-changed'\)\);/);

  const bridge = read('skyblock-redesign-bridge.js');
  assert.doesNotMatch(bridge, /nav-link\[data-page="tools"\][\s\S]*\.click\s*\(/);
});

test('normal renders preserve the relative scroll anchor', () => {
  const source = read('app.js');
  assert.match(source, /const relativeAnchor = preserveScroll \? currentScrollAnchor\(\)/);
  assert.match(source, /restoreRelativeScrollAnchor\(relativeAnchor\);/);
  assert.match(source, /scheduleScrollAnchorRestore\(relativeAnchor\);/);
  assert.match(source, /render\(\{ preserveScroll: false \}\)/, 'real navigation still opts out of scroll preservation');
});
