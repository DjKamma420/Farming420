...
test('an unrecognised payload yields an empty catalogue rather than nonsense', () => {
  for (const payload of [null, {}, { items: 'nope' }, { data: [] }]) {
    assert.deepEqual(reduceItemResource(payload), []);
  }
});
...
test("reforge options come from the data layer and the player's own items", () => {
  const fromData = reforgeOptions(null);
  assert.ok(fromData.some(option => option.value === 'mossy' && option.source === OPTION_SOURCE.APP_DATA));
  assert.ok(fromData.some(option => option.value === 'beady'), 'entry ids are parsed, not display names');

  const withProfile = reforgeOptions({ items: [{ reforge: 'Blooming' }] });
  const blooming = withProfile.find(option => option.value === 'blooming');
  assert.equal(blooming.source, OPTION_SOURCE.PROFILE, 'a reforge the player actually has is offered');
});

test('a value present in both sources is listed once, attributed to the profile', () => {
  const options = reforgeOptions({ items: [{ reforge: 'mossy' }] });
  const mossy = options.filter(option => option.value === 'mossy');
  assert.equal(mossy.length, 1);
  assert.equal(mossy[0].source, OPTION_SOURCE.PROFILE);
});

test('enchantment and gem options come from the decoded items', () => {
...
test('a stale cache is refreshed and rewritten', async () => {
  ...
  const result = await loadItemCatalog({
    fetchImpl: async () => ({ ok: true, status: 200, json: async () => RESOURCE }),
  });
...
test('the resource is requested keylessly from the official endpoint', async () => {
  ...
  await loadItemCatalog({
    fetchImpl: async (url, init) => {
      seen.push({ url, init });
      return { ok: true, status: 200, json: async () => RESOURCE };
    },
  });
  assert.equal(seen[0].url, 'https://api.hypixel.net/v2/resources/skyblock/items');
  assert.equal(seen[0].init.headers['API-Key'], undefined, 'the item resource needs no key');
});
...
test('a failed fetch keeps a stale cache and reports the reason', async () => {
  ...
  assert.equal(result.items.length, 1, 'a stale list beats an empty picker');
  assert.equal(result.fromCache, true);
