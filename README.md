...
- installable PWA foundation; the legacy cache-first service worker is retired so it cannot pin stale builds
- each Pages deployment is stamped with its immutable Git commit SHA and exposed through `deploy-version.json`
- the browser checks that deployment marker on startup and when the app regains focus, then reloads a newer build when it is safe to do so
- deployment-time asset URLs are cache-busted from the commit SHA; ordinary code changes require no manual service-worker or app-version bump
...
- JavaScript validation plus service-worker retirement and update-delivery tests through GitHub Actions
...
- deployment identity comes from the Git commit SHA generated by Pages CI, not from a manually edited cache/version constant
- legacy Farming420 service-worker registrations and Cache Storage entries are removed before switching to a newer deployment
- a failed update check leaves the currently loaded application untouched
...
A local HTTP server is required for module loading and PWA testing. The commit-derived `deploy-version.json` marker is generated only by the GitHub Pages deployment workflow.
...
GitHub Pages deploys automatically on every push to `main` and can also be triggered manually. The workflow stamps the deployed `index.html` and local asset URLs with `${{ github.sha }}`, then writes the same SHA to `deploy-version.json`. Runtime update checks compare those two values, so normal releases do not require a manual service-worker version bump.
