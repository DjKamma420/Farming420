  ...
  if (root && typeof MutationObserver === 'function') {
    new MutationObserver(mutations => {
      const relevant = mutations.some(mutation => [...mutation.addedNodes].some(node =>
        ...
        && (node.matches?.(`.item-card, .drawer, .sb-reforge-card, .workspace-level-row, .item-editor-row, .accessory-upgrade-row, ${RECOMBOBULATOR_CONTROL_SELECTOR}`)
          || node.querySelector?.(`.item-card, .drawer, .sb-reforge-card, .workspace-level-row, .item-editor-row, .accessory-upgrade-row, ${RECOMBOBULATOR_CONTROL_SELECTOR}`))));
