...
function cowSynergyContext() {
  ...
  const strength = state.profile?.inputs?.strength;
  return {
    cow,
    strength: strength === null || strength === undefined || strength === '' ? null : Number(strength),
  };
}
...
function shardSynergyPanel() {
  ...
  return `<section class="accessory-model-note shard-synergy-panel">
    ...
    <span>These effects are kept separate from flat Farming Fortune. ${cowReady
